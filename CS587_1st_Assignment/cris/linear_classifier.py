...
class LinearClassifier(object):
  ...
  def train_linear_classifier(self, X_train, y_train, X_val, y_val, learning_rate=1e-3, reg=1e-5, reg_type = 2, num_epochs = 6, batch_size = 200, num_valid_loss_evals = 100, verbose=False):
	...
	#array to save the computed training loss 
    train_loss_history = []
    ...
    for it in xrange(num_iters):
      ...
      X_batch = X_train[batch_indexes, :]       # take only the batch samples
      y_batch = y_train[batch_indexes]
      #########################################################################
      #                       END OF YOUR CODE                                #
      #########################################################################		
      # evaluate loss and gradient
      train_loss, grad = self.loss(X_batch, y_batch, reg, reg_type, 0) # calc the grad
      train_loss_history.append(train_loss) # keep the loss value in the list
      
      if it > num_valid_loss_evals: # later calculation, after 100 
          valid_loss, val_grad = self.loss(X_val, y_val, 0, reg_type, 1) # don't calc the grad, don't add regularizer 
          valid_loss_history.append(valid_loss)    # keep the validation loss value in the list  
...
#      self.W /= batch_size
      ...
      #########################################################################
#    self.W /= batch_size  
    print 'CSFAK INSIDE train_linear_classifier'
  ...
  def predict_image_class(self, X):
    ...
    - y_pred: Predicted labels for the data in X. y_pred is a 1-dimensional
      ...
      class.
    """
    y_pred = np.zeros(X.shape[1]) # zero initilize 
    scores = X.dot(self.W) # dot product the test sample with the weights
    y_pred = np.argmax(scores, axis = 1) # keep only the argmax predictions...
    ###########################################################################
    # TODO:                                                                   #
    ...
    #                           END OF YOUR CODE                              #
    ###########################################################################
    print 'CSFAK INSIDE predict_image_class'
    return y_pred
